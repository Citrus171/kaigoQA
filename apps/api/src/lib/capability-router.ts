...
// ── knowledge_qa 生成: V2 EDGE_SYSTEM ベース（数値を省略させない）。
//   2026-06-22 real-query eval(out/eval-rag-gen): system-explain で「3〜5文」制約が
//   複数の事実要点を取りこぼし partial を誘発（good 1/6）と判明。網羅性優先に変更。
//   これにより本番 rag-mvp.py の V2 とは brevity 文のみ乖離する（数値省略禁止の核は維持）。
export const KNOWLEDGE_QA_SYSTEM =
  ...
  "利用者の判断に必要な事実・条件・数値は省略せず網羅的に回答してください。簡潔さより網羅性を優先し、" +
  "文数は内容に応じて調整してください（要点が多い質問では6〜8文でもよい）。" +
