...
EDGE_OUT       = P("apps/api/eval/data/rag-mvp-edge-thinkon-rearag.jsonl")
CLOUD_OUT      = P("apps/api/eval/data/rag-mvp-cloud.jsonl")
RESULT_MD      = P("apps/api/eval/out/43-edge-thinkon-rag-remeasure.md")
EDGE_ORACLE_P  = os.environ.get(
    "EDGE_ORACLE_PATH",
    P("apps/api/eval/data/phaseA-gemma4-incontext-results-edge-thinkon-v2.json"),
)
...
def gen_edge(query, refs):
    """Gemma4 26B Workers AI, thinkON + V2"""
...
def run_layer2(gold_dict, corpus, corpus_embeds):
    ...
    for label, out_path, gen_fn in [
        ...
        print(f"\n── {label} (model={'Gemma4 thinkON+V2' if label=='edge' else CLOUD_MODEL}) ──")
...
def layer3_analysis():
    ...
    edge_result = two_by_two(edge_oracle, edge_real, "Edge (Gemma4 thinkON+V2)")
...
def main():
    ...
    elapsed = time.time() - t0
    md = []
    md.append("# 43: edge(Gemma4 thinkON+V2) 実RAG k=5 再測定")
    md.append("")
    ...
    md.append(f"- edge model: Gemma4 26B thinkON+V2 (Workers AI)")
